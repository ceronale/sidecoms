...
{	
    ...
    public function dataview_clientes($query)
    {
            ...
            while($row=$stmt->fetch(PDO::FETCH_ASSOC))
                {
                    ?>
<tr>
	<td><?php print($row['razon_social_c']); ?></td>
    <td><?php print($row['rif_c']); ?></td>
    ...
    <td><?php print($row['nombres_c'].' '.$row['apellidos_c']); ?></td>
    <td><?php print($row['documento_c']); ?></td>
    <td><?php print($row['email_c']); ?></td>
    <td><?php print($row['telefono_c']); ?></td>
...
<?php
	...
	//FIN FUNCION PARA MOSTRAR LISTADO DE CLIENTES

	//FUNCION PARA CREAR UN CLIENTE EN LA BD
    public function crear_cliente($nombres,$apellidos,$nacionalidad,$documento,$sexo,$dob,$usuario,$pass,$email,$telefono,$razon_social,$rif,$notas)
	{
		try
		{
            $id_tipo_usuario = "2";
            ...
            $stmt2 = $this->conn->prepare("INSERT INTO clientes(id_usuario,documento,nombres,apellidos,dob,nacionalidad,sexo,email,telefono,razon_social,rif,notas,created_at,updated_at) 
				VALUES(:id_usuario,:documento,:nombres,:apellidos,:dob,:nacionalidad,:sexo,:email,:telefono,:razon_social,:rif,:notas,:created_at,:updated_at)");
			...
			$operacion = "Se registro el usuario (cliente): " . $last_id;
	...
	//FIN FUNCION PARA CREAR UN CLIENTE EN LA BD

	//FUNCION PARA EDITAR UN CLIENTE EN LA BD
    public function editar_cliente($nombres,$apellidos,$nacionalidad,$documento,$sexo,$dob,$usuario,$pass,$email,$telefono,$razon_social,$rif,$notas,$id_usuario)
	{
		try
		{
            $updated_at = date("Y-m-d H:i:s", strtotime('now'));
			...
			$stmt2=$this->conn->prepare("UPDATE clientes SET 
				documento=:documento,
				sexo=:sexo, 
				nombres=:nombres,  
                apellidos=:apellidos, 
			...
			$operador = $_SESSION['user_session'];
			$operacion = "Se modificaron los datos del usuario(cliente): " . $id_usuario;
			$ip = $_SERVER['REMOTE_ADDR'];
			$this->log_operaciones($operador,$operacion,$usuario,$ip);
	...
	//FIN FUNCION PARA EDITAR UN CLIENTE EN LA BD

	//FUNCION PARA ELIMINAR UN CLIENTE DE LA BD Y GUARDARLO EN LA TABLA DE RESPALDO
    public function eliminar_cliente($id_usuario)
	{  
            extract($this->get_usuarioID($id_usuario));	
            extract($this->get_clienteidusuario($id_usuario));	
            $fecha_eliminado= date("Y-m-d H:i:s", strtotime('now'));
            
            $stmt2 = $this->conn->prepare("INSERT INTO clientes_eliminados(id_usuario,documento,nombres,apellidos,dob,nacionalidad,sexo,email,telefono,razon_social,rif,notas,usuario,pass,tipo_usuario,created_at,updated_at,fecha_eliminado) 
				VALUES(:id_usuario,:documento,:nombres,:apellidos,:dob,:nacionalidad,:sexo,:email,:telefono,:razon_social,:rif,:notas,:usuario,:pass,:tipo_usuario,:created_at,:updated_at,:fecha_eliminado)");
			$stmt2->bindparam(":id_usuario",$id_usuario);
			$stmt2->bindparam(":documento",$documento);
			$stmt2->bindparam(":nombres",$nombres);
			$stmt2->bindparam(":apellidos",$apellidos);
            ...
            $stmt3 = $this->conn->prepare("DELETE FROM clientes WHERE id_usuario=:id_usuario");
			$stmt3->bindparam(":id_usuario",$id_usuario);
			$stmt3->execute();

			$operador = $_SESSION['user_session'];
			$operacion = "Se eliminó el usuario(cliente): " . $id_usuario;
			$ip = $_SERVER['REMOTE_ADDR'];
			$this->log_operaciones($operador,$operacion,$usuario,$ip);
			
			return true;
	}
	//FIN FUNCION PARA ELIMINAR UN CLIENTE DE LA BD Y GUARDARLO EN LA TABLA DE RESPALDO
